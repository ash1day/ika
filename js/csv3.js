...
var depth_csv = 30;
var day_csv = 10;//ヒートマップがどんだけ遡れる情報盛ってるか
var day_csv_pathline = 30;/////何日前までさかのぼれる情報盛ってるか
var points = [];
var axis = ["X","Y","Z"]



function point(year,month,day,lat,lon,cpue,xyz,S_pathlines,S,T,U,V,W) {
    this.year = year;
    this.month = month;
    this.day = day;
    this.lat = lat;
...
for (i_p = 0; i_p < alldata.length; i_p++) {//points
...
var HM_csv = [];
        ...
        for (i = 0; i < depth_csv; i++) {//depth
            ...
            for (j = 0; j < day_csv_pathline+1; j++) {//day   day_csv_pathline+1  何か1足さないと
                x_csv[i].push([]);
                y_csv[i].push([]);
                z_csv[i].push([]);
                xy_csv[i].push([]);
...
//console.log(x_csv)
        ...
        for (i = 0; i < depth_csv; i++) {//depth
                x_csv[i][0].push(Number(alldata[i_p][4]));
                y_csv[i][0].push(Number(alldata[i_p][3]));
                z_csv[i][0].push(Number(scale_z_lev[i]));
        }



for (i = 0; i < alldata[row_label].length; i++) {
                        ...
                        if  (alldata[row_label][i].slice(1,3) == name_day) {//day determin
                            if  (i_ax == 0) {//day determin
                              ...
                              //  console.log("x_csv[i_d][i_d2+1] = " + x_csv[i_d][i_d2+1])                                
                                ...
                                y_csv[i_d][i_d2+1].push(Number(alldata[i_p][i]));
                                }
...
for (i = 0; i < depth_csv; i++) {//depth
    for (j = 0; j < day_csv_pathline; j++) {//day
                          ...
                          //  console.log(i_v)
                            ...
                            if  (i_v == 0) {//day determin
                              ...
                              //  console.log("x_csv[i_d][i_d2+1] = " + x_csv[i_d][i_d2+1])                                
                                ...
                                T_csv[i_d][i_d2].push(Number(alldata[i_p][i]));
                                }
                            else if  (i_v == 2) {//day determin
                                U_csv[i_d][i_d2].push(Number(alldata[i_p][i]));
                                }
                            else if  (i_v == 3) {//day determin
                                V_csv[i_d][i_d2].push(Number(alldata[i_p][i]));
                                }
...
points[i_p] = new point(Number(alldata[i_p][0]),Number(alldata[i_p][1]),Number(alldata[i_p][2]),Number(alldata[i_p][3]),Number(alldata[i_p][4]),Number(alldata[i_p][5]),xy_csv, S_pathline2,S_csv,T_csv,U_csv,V_csv,W_csv);
