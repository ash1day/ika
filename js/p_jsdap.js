...
var svg_ocean = d3.select("#mainocean25555").append("svg").attr({
    width: w,
    height: h
}).call(zoom);
...
function outputData_ocean() {
    enter_ocean.append("circle").attr({
        cx: function(d, i) {
        ...
        r: function(d, i) {
            return get_R_2d_ocean(d[3]);
        },
        fill: function(d, i) {
            return get_color_target_2d_ocean(d[3]);
        }
    });
}
...
function get_Scale_S_color(minS, maxS) {
    Scale_S_color = d3.scale.linear().domain([minS, maxS]).range([v_minS, v_maxS]).nice();
}

function get_Scale_T_color(minT, maxT) {
    Scale_T_color = d3.scale.linear().domain([minT, maxT]).range([v_minT, v_maxT]).nice();
}
...
function draw_points_2d_ocean() { //////海書くだけ
    ...
    console.log(i_date_jsdap)
    jqdap.loadData("http://hyperinfo.viz.media.kyoto-u.ac.jp/~futami/dias/MOVE-RA2014.dods?" + variable + "[" + (i_date_jsdap - 732313) + "][" + z_lev_2dfiz + "][" + start_scale_y_lat + ":" + end_scale_y_lat + "][" + start_scale_x_lon + ":" + end_scale_x_lon + "]", {
...
function draw_points_2d_ocean_pathline(det_day_3d) { ////1つの変数の描画　とパスライン
    ...
    jqdap.loadData("http://hyperinfo.viz.media.kyoto-u.ac.jp/~futami/dias/MOVE-RA2014.dods?" + variable + "[" + (i_date_jsdap - 732313) + "][" + z_lev_2dfiz + "][" + start_scale_y_lat + ":" + end_scale_y_lat + "][" + start_scale_x_lon + ":" + end_scale_x_lon + "]", {
        ...
        removeData_ocean2()
        updateData_ocean2()
        g_ocean.selectAll("text").data(xyzvar).enter().append("text");
        g_ocean.selectAll("text").data(xyzvar).text(function(d, i) {
...
var fishingpointscpue_gloval = [];
//draw_points_2d_ocean_pathline_suikai(1)
svg_pathline_2d_zoom.append("g").attr({
    class: "axis",
...
function draw_points_2d_ocean_pathline_suikai(det_day_3d) { ////水塊と　パスラインと　海　リク
    ...
    jqdap.loadData("http://hyperinfo.viz.media.kyoto-u.ac.jp/~futami/dias/MOVE-RA2014.dods?" + "T" + "[" + (i_date_jsdap - 732313) + "][" + z_lev_2dfiz + "][" + start_scale_y_lat + ":" + end_scale_y_lat + "][" + start_scale_x_lon + ":" + end_scale_x_lon + "]", {
        withCredentials: true
    }).then(function(data_jsdap_T) {
        jqdap.loadData("http://hyperinfo.viz.media.kyoto-u.ac.jp/~futami/dias/MOVE-RA2014.dods?" + "S" + "[" + (i_date_jsdap - 732313) + "][" + z_lev_2dfiz + "][" + start_scale_y_lat + ":" + end_scale_y_lat + "][" + start_scale_x_lon + ":" + end_scale_x_lon + "]", {
            withCredentials: true
                ...
                for (var y_js = start_scale_y_lat; y_js <= end_scale_y_lat; y_js++) {
                    for (var x_js = start_scale_x_lon; x_js <= end_scale_x_lon; x_js++) {
                        xyzvar.push([scale_x_lon[x_js], scale_y_lat[y_js], scale_z_lev[z_js],
                            ...
                            get_v_dence(scale_z_lev[z_js], data_jsdap_S[0][0][0][z_js - start_scale_z_lev][y_js - start_scale_y_lat][x_js - start_scale_x_lon], data_jsdap_T[0][0][0][z_js - start_scale_z_lev][y_js - start_scale_y_lat][x_js - start_scale_x_lon])
                        ]); //x,y,z,var
...
*/
    function draw_pathline_daydev_remove(temp_xyin2) {
        svg_pathline_2d.selectAll("path").remove()
        svg_pathline_2d_zoom.selectAll("path").remove()
    }
    ...
    function draw_pathline_daydev(temp_xyin2) {
            ...
            line = d3.svg.line()
                // interpolate指定で点のつなぎ方を指定する。
                .interpolate('basis').x(function(d) {
                    return xScale_zoom(d[0]) - 0;
                }).y(function(d) {
                    return yScale_zoom(d[1]) - 0;
                });
...
function updateData_ocean2() {
    update_ocean.attr({
        ...
        r: function(d, i) {
            return get_R_2d_ocean2(d[3], i);
        },
        fill: function(d, i) {
            return get_color_target_2d_ocean2(d[3], i);
        }
    });
}

function outputData_pathline_text() {
    enter_pathline_text.append("text").text(function(d, i) {
        return get_text_pathline(i);
    }).attr("x", function(d) {
        return xScale(d[0]);
    }).attr("y", function(d) {
        return yScale(d[1]);
    }).attr("font-family", "sans-serif").attr("font-size", "22px").attr("fill", "black");
}

function removeData_pathline_text() {
    exit_pathline_text.remove();
}

function updateData_pathline_text() {
    update_pathline_text.append("text").text(function(d, i) {
        return get_text_pathline(i);
    }).attr("x", function(d) {
        return xScale(d[0]);
    }).attr("y", function(d) {
...
function updateData_ocean2_suikai() {
    update_ocean.attr({
        ...
        r: function(d, i) {
            return get_R_2d_ocean2_suikai(i, d[3], d[4], d[5]);
        },
        fill: function(d, i) {
            return get_color_target_2d_ocean2_suikai(i, d[3], d[4], d[5]);
        },
        stroke: function(d, i) {
            return get_strokcolor_target_2d_ocean2_suikai(i, d[3], d[4], d[5]);
        }
    });
}
...
function get_R_2d_ocean2_suikai(i, salinity, tempereture, dence) {
    ...
    if (i < length_d_ocean) {
        ...
        } else if ((minT < tempereture) && (tempereture < maxT) && (minS < salinity) && (salinity < maxS) && (mindence < dence) && (dence < maxdence)) {
            R_2d_suikai = average_R;
        } else {
            R_2d_suikai = average_R;
        };
...
function get_color_target_2d_ocean2_suikai(i, salinity, tempereture, dence) {
    ...
    if (i < length_d_ocean) {
        if (salinity < thre_island_S) { //大陸
            ...
            && (min_S_OW < salinity) && (salinity < max_S_OW) && (min_dence_OW < dence) && (dence < max_dence_OW)) {
            code = "#6495ED";
        } else if ((min_T_KW < tempereture) && (tempereture < max_T_KW) //KW
            && (min_S_KW < salinity) && (salinity < max_S_KW) && (min_dence_KW < dence) && (dence < max_dence_KW)) {
            code = "#FFA07A";
        } else {
            //r = parseInt(Scale_S_color(val));
            code = "#F0F8FF";
...
function get_strokcolor_target_2d_ocean2_suikai(i, salinity, tempereture, dence) {
        if (i < length_d_ocean) {} else if (length_d_ocean <= i) {
            //return "#7FFFD4";
            return;
        }
    }
...
function updateData_ocean2_suikai_T() {
    update_ocean.attr({
        ...
        r: function(d, i) {
            return get_R_2d_ocean2_suikai_T(i, d[3]);
        },
        fill: function(d, i) {
            return get_color_target_2d_ocean2_suikai_T(i, d[3]);
        },
        stroke: function(d, i) {
            return get_strokcolor_target_2d_ocean2_suikai_T(i, d[3]);
        }
    });
}
...
function outputData_pathline_circle() {
    enter_pathline_circle.append("circle")
}

function removeData_pathline_circle() {
    exit_pathline_circle.remove();
}

function updateData_pathline_circle() {
    update_pathline_circle.on("click", function(d, i) {
        // change_heat2d ( d[0],d[1] )
        console.log("rrr")
            // day( i%3 )
        ;
        ...
        r: function(d, i) {
            return get_R_2d_pathline_first(i, d[3], d[4], d[5], d[6], d[7]);
        }, ///3:cpue,4:S,5:T,6:updawn,7:z
        fill: function(d, i) {
            return get_pathline_circle_color(i, d[3], d[4], d[5], d[6], d[7]);
        }, ///3:cpue,4:S,5:T,6:updawn,7:z
        stroke: function(d, i) {
                return get_strokcolor_pathline_circle(i, d[3]);
            }
...
function outputData_pathline_circle_zoom() {
    enter_pathline_circle_zoom.append("circle")
}

function removeData_pathline_circle_zoom() {
    exit_pathline_circle_zoom.remove();
}

function updateData_pathline_circle_zoom() {
    update_pathline_circle_zoom.on("click", function(d, i) {
        // change_heat2d ( d[0],d[1] )
        ...
        r: function(d, i) {
            return get_R_2d_pathline_first_zoom(i, d[3], d[4], d[5], d[6], d[7]);
        }, ///3:cpue,4:S,5:T,6:updawn,7:z
        fill: function(d, i) {
            return get_pathline_circle_color(i, d[3], d[4], d[5], d[6], d[7]);
        }, ///3:cpue,4:S,5:T,6:updawn,7:z
...
function get_strokecolor_pathline_total(i, cpue, S, T, updown, z) {
    if (pathlinecolormode == 0) { //CPUE
        ...
        if ((min_T_OW < T) && (T < max_T_OW) //OW
            && (min_S_OW < S) && (S < max_S_OW) && (min_dence_OW < get_v_dence(z, S, T)) && (get_v_dence(z, S, T) < max_dence_OW)) {
            suikai_kari = 240;
            //   console.log("OW")
            ...
            && (min_S_KW < S) && (S < max_S_KW) && (min_dence_KW < get_v_dence(z, S, T)) && (get_v_dence(z, S, T) < max_dence_KW)) {
            suikai_kari = 359;
            console.log("KW")
        } else {
            suikai_kari = 300;
...
var min_cpue = 0;
var max_cpue = 35;
Scale_cpue = d3.scale.linear().domain([min_cpue, max_cpue]).range([1.5, 3.0]).nice();
Scale_cpue_zoom = d3.scale.linear().domain([min_cpue, max_cpue]).range([2, 4.0]).nice();
Scale_cpue_pathline_width = d3.scale.linear().domain([min_cpue, max_cpue]).range([0.5, 1]).nice();
Scale_cpue_pathline_width_zoom = d3.scale.linear().domain([min_cpue, max_cpue]).range([2, 4]).nice();
Scale_color_cpue = d3.scale.linear().domain([min_cpue, max_cpue]).range([0, 1]).nice();
Scale_cpue_HSV_H = d3.scale.linear().domain([min_cpue, max_cpue]).range([240, 359]).nice();
Scale_cpue_HSV_S = d3.scale.linear().domain([min_cpue, max_cpue]).range([0, 1]).nice();
var min_S = 32.5;
var max_S = 34.5;
Scale_S_HSV_H = d3.scale.linear().domain([min_S, max_S]).range([240, 359]).nice();
var min_T = 0;
var max_T = 25;
Scale_T_HSV_H = d3.scale.linear().domain([min_T, max_T]).range([240, 359]).nice();
var min_updown = -0.2;
var max_updown = 0.2;
Scale_updown_HSV_H = d3.scale.linear().domain([min_updown, max_updown]).range([240, 359]).nice();
...
function HLStoRGB(h, l, s) {
    ...
    ///////////////////// sort color code
var svg2_pathlinesort = d3.select("#Sortlabel_total").append("svg").attr({
    width: 100,
    height: 100
});
color = [0, 0];
...
function colorcode_sort_info() {
    ...
    gradient = svg2_pathlinesort.append("svg:defs").append("svg:linearGradient").attr("id", "gradient").attr("x1", "0%").attr("y1", "0%").attr("x2", "0%").attr("y2", "100%").attr("x3", "0%").attr("y3", "0%").attr("x4", "0%").attr("y4", "0%")
    gradient.append("svg:stop").attr("offset", "0%").attr("stop-color", get_strokecolor_pathline_total(0, max_cpue, max_S, max_T, max_updown, 0)).attr("stop-opacity", 1)
    ...
    gradient.append("svg:stop").attr("offset", "100%").attr("stop-color", get_strokecolor_pathline_total(0, min_cpue, min_S, min_T, min_updown, 0)).attr("stop-opacity", 1)
}

function create_colorcode_sort_info() {
    console.log("create_colorcode_sort_info")
    svg2_pathlinesort.selectAll("rect").data(color).enter().append("rect").attr({
        x: function(d, i) {
            return 10;
        },
        y: function(d, i) {
